/*===-- test/test_vectors/block.c ----------------------*- PUBLIC -*- C -*-===*/
/**
*** @file
*** @brief Test Vectors
***
*** This unit provides test vectors for block ciphers.
**/
/*===----------------------------------------------------------------------===*/
...
static int check(struct TEST_VECTOR test)
{
    ...
    block_len = block_query(test.primitive, BLOCK_SIZE_Q, 0);

    ASSERT_SUCCESS(block_init(&state, test.key, test.key_len,
                              test.primitive, 0));

    memcpy(scratch, test.input, block_len);

    block_forward(&state, scratch);

    ASSERT_BUF_EQ(scratch, test.expected, block_len);

    block_inverse(&state, scratch);

    ASSERT_BUF_EQ(scratch, test.input, block_len);
...
int test_vectors_aes(void)
{
    return check_generic(BLOCK_AES);
}

int test_vectors_nullcipher(void);
int test_vectors_nullcipher(void)
{
    return check_generic(BLOCK_NULLCIPHER);
}

int test_vectors_threefish256(void);
int test_vectors_threefish256(void)
{
    return check_generic(BLOCK_THREEFISH256);
}
