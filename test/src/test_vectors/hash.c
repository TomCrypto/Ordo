/*===-- test/test_vectors/hash.c -----------------------*- PUBLIC -*- C -*-===*/
/**
*** @file
*** @brief Test Vectors
***
*** This unit provides test vectors for hash functions.
**/
/*===----------------------------------------------------------------------===*/
...
{
    size_t len = digest_length(test.primitive);
    ...
    ASSERT_SUCCESS(hash_init(&state, test.primitive, 0));

    hash_update(&state, test.input, test.input_len);

    hash_final(&state, &digest);

    ASSERT_BUF_EQ(digest, test.expected, len);
...
int test_vectors_md5(void)
{
    return check_generic(HASH_MD5);
}

int test_vectors_sha256(void);
int test_vectors_sha256(void)
{
    return check_generic(HASH_SHA256);
}

int test_vectors_skein256(void);
int test_vectors_skein256(void)
{
    return check_generic(HASH_SKEIN256);
}
