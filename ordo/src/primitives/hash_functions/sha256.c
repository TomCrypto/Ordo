...
const uint32_t constants[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
...
int SHA256_Init(HASH_FUNCTION_CONTEXT* ctx, void* params)
{
    ...
    memcpy(state(ctx)->digest, initialState, SHA256_DIGEST);
...
inline void SHA256_Compress(uint32_t block[16], uint32_t digest[8])
{
    ...
    for (t = 0; t < 64; t++)
    {
        ...
        t1 = h + (ROR(e, 6) ^ ROR(e, 11) ^ ROR(e, 25)) + ((e & f) ^ ((~e) & g)) + constants[t] + w[t];
